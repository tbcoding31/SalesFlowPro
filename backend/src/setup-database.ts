...
async function setupDatabase() {
  ...
  const tableQueries = [
    ...
    `CREATE TABLE IF NOT EXISTS visit_reminder_defaults (id VARCHAR(50) PRIMARY KEY, dashboardReminderEnabled BOOLEAN NOT NULL DEFAULT TRUE, dashboardReminderDaysBefore INT NOT NULL DEFAULT 5, emailReminderEnabled BOOLEAN NOT NULL DEFAULT TRUE, emailReminderDaysBefore INT NOT NULL DEFAULT 2, immediateReminderInsideWindowEnabled BOOLEAN NOT NULL DEFAULT TRUE, isActive BOOLEAN NOT NULL DEFAULT TRUE, createdAt DATETIME DEFAULT CURRENT_TIMESTAMP, updatedAt DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, updatedById VARCHAR(50))`,
    `CREATE TABLE IF NOT EXISTS tenant_visit_reminder_settings (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NOT NULL UNIQUE, dashboardReminderEnabled BOOLEAN NOT NULL, dashboardReminderDaysBefore INT NOT NULL, emailReminderEnabled BOOLEAN NOT NULL, emailReminderDaysBefore INT NOT NULL, immediateReminderInsideWindowEnabled BOOLEAN NOT NULL, createdAt DATETIME DEFAULT CURRENT_TIMESTAMP, updatedAt DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, updatedById VARCHAR(50), CONSTRAINT fk_tvrs_tenant FOREIGN KEY (tenantId) REFERENCES tenants(id) ON DELETE CASCADE)`,
    `CREATE TABLE IF NOT EXISTS visit_reminder_deliveries (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NOT NULL, visitId VARCHAR(50) NOT NULL, recipientUserId VARCHAR(50), recipientEmail VARCHAR(255) NOT NULL, recipientRole VARCHAR(50) NOT NULL, reminderType VARCHAR(50) NOT NULL DEFAULT 'EMAIL_REMINDER', visitDate DATE NOT NULL, visitStartTime TIME NOT NULL, scheduledOffsetDays INT NOT NULL, deliveryStatus VARCHAR(20) NOT NULL DEFAULT 'PENDING', attemptCount INT NOT NULL DEFAULT 0, lastAttemptAt DATETIME, nextAttemptAt DATETIME, sentAt DATETIME, errorMessage TEXT, idempotencyKey VARCHAR(255) NOT NULL UNIQUE, createdAt DATETIME DEFAULT CURRENT_TIMESTAMP, updatedAt DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, INDEX idx_vrd_tenant_visit (tenantId, visitId), INDEX idx_vrd_status_next (deliveryStatus, nextAttemptAt), INDEX idx_vrd_tenant_date (tenantId, visitDate))`,
    
    // USERS & ORG
    `CREATE TABLE IF NOT EXISTS users (id VARCHAR(50) PRIMARY KEY, email VARCHAR(255) UNIQUE, name VARCHAR(255), phone VARCHAR(50), location VARCHAR(100), timezone VARCHAR(50), passwordHash VARCHAR(255), avatar VARCHAR(500), status VARCHAR(20), lastLoginAt DATETIME, passwordChangedAt DATETIME, createdAt DATETIME DEFAULT CURRENT_TIMESTAMP)`,
    ...
    `CREATE TABLE IF NOT EXISTS departments (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NULL, sourceType ENUM('PLATFORM', 'TENANT') NOT NULL DEFAULT 'PLATFORM', platformMasterId VARCHAR(50) NULL, name VARCHAR(255), description TEXT, isActive TINYINT(1) NOT NULL DEFAULT 1, displayOrder INT NOT NULL DEFAULT 0, masterScopeKey VARCHAR(70) GENERATED ALWAYS AS (COALESCE(tenantId, '__PLATFORM__')) STORED, INDEX idx_departments_tenant (tenantId))`,
    `CREATE TABLE IF NOT EXISTS positions (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NULL, sourceType ENUM('PLATFORM', 'TENANT') NOT NULL DEFAULT 'PLATFORM', platformMasterId VARCHAR(50) NULL, name VARCHAR(255), level INT DEFAULT 1, isActive TINYINT(1) NOT NULL DEFAULT 1, masterScopeKey VARCHAR(70) GENERATED ALWAYS AS (COALESCE(tenantId, '__PLATFORM__')) STORED, INDEX idx_positions_tenant (tenantId))`,
    ...
    // MASTER DATA (Lookups — Two-Tier Scoped Master Tables)
    `CREATE TABLE IF NOT EXISTS activity_types (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NULL, sourceType ENUM('PLATFORM', 'TENANT') NOT NULL DEFAULT 'PLATFORM', platformMasterId VARCHAR(50) NULL, code VARCHAR(50), name VARCHAR(100), icon VARCHAR(50), color VARCHAR(50), isActive TINYINT(1) NOT NULL DEFAULT 1, displayOrder INT NOT NULL DEFAULT 0, masterScopeKey VARCHAR(70) GENERATED ALWAYS AS (COALESCE(tenantId, '__PLATFORM__')) STORED, UNIQUE KEY uq_activity_types_scope_code (masterScopeKey, code), INDEX idx_activity_types_tenant (tenantId))`,
    `CREATE TABLE IF NOT EXISTS customer_statuses (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NULL, sourceType ENUM('PLATFORM', 'TENANT') NOT NULL DEFAULT 'PLATFORM', platformMasterId VARCHAR(50) NULL, code VARCHAR(50), name VARCHAR(100), color VARCHAR(50), isActive TINYINT(1) NOT NULL DEFAULT 1, displayOrder INT NOT NULL DEFAULT 0, masterScopeKey VARCHAR(70) GENERATED ALWAYS AS (COALESCE(tenantId, '__PLATFORM__')) STORED, UNIQUE KEY uq_customer_statuses_scope_code (masterScopeKey, code), INDEX idx_customer_statuses_tenant (tenantId))`,
    `CREATE TABLE IF NOT EXISTS customer_types (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NULL, sourceType ENUM('PLATFORM', 'TENANT') NOT NULL DEFAULT 'PLATFORM', platformMasterId VARCHAR(50) NULL, code VARCHAR(50), name VARCHAR(100), isActive TINYINT(1) NOT NULL DEFAULT 1, displayOrder INT NOT NULL DEFAULT 0, masterScopeKey VARCHAR(70) GENERATED ALWAYS AS (COALESCE(tenantId, '__PLATFORM__')) STORED, UNIQUE KEY uq_customer_types_scope_code (masterScopeKey, code), INDEX idx_customer_types_tenant (tenantId))`,
    `CREATE TABLE IF NOT EXISTS follow_up_types (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NULL, sourceType ENUM('PLATFORM', 'TENANT') NOT NULL DEFAULT 'PLATFORM', platformMasterId VARCHAR(50) NULL, code VARCHAR(50), name VARCHAR(100), isActive TINYINT(1) NOT NULL DEFAULT 1, displayOrder INT NOT NULL DEFAULT 0, masterScopeKey VARCHAR(70) GENERATED ALWAYS AS (COALESCE(tenantId, '__PLATFORM__')) STORED, UNIQUE KEY uq_follow_up_types_scope_code (masterScopeKey, code), INDEX idx_follow_up_types_tenant (tenantId))`,
    `CREATE TABLE IF NOT EXISTS project_stages (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NULL, sourceType ENUM('PLATFORM', 'TENANT') NOT NULL DEFAULT 'PLATFORM', platformMasterId VARCHAR(50) NULL, code VARCHAR(50), name VARCHAR(100), phase VARCHAR(50) DEFAULT 'PIPELINE', commercialOutcome ENUM('OPEN', 'WON', 'LOST') DEFAULT 'OPEN', displayOrder INT NOT NULL DEFAULT 0, probability INT NOT NULL DEFAULT 0, isTerminal TINYINT(1) NOT NULL DEFAULT 0, allowVisits TINYINT(1) NOT NULL DEFAULT 1, allowNewProject TINYINT(1) NOT NULL DEFAULT 1, isActive TINYINT(1) NOT NULL DEFAULT 1, masterScopeKey VARCHAR(70) GENERATED ALWAYS AS (COALESCE(tenantId, '__PLATFORM__')) STORED, UNIQUE KEY uq_project_stages_scope_code (masterScopeKey, code), INDEX idx_project_stages_tenant (tenantId))`,
    `CREATE TABLE IF NOT EXISTS task_priorities (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NULL, sourceType ENUM('PLATFORM', 'TENANT') NOT NULL DEFAULT 'PLATFORM', platformMasterId VARCHAR(50) NULL, code VARCHAR(50), name VARCHAR(100), color VARCHAR(50), isActive TINYINT(1) NOT NULL DEFAULT 1, displayOrder INT NOT NULL DEFAULT 0, masterScopeKey VARCHAR(70) GENERATED ALWAYS AS (COALESCE(tenantId, '__PLATFORM__')) STORED, UNIQUE KEY uq_task_priorities_scope_code (masterScopeKey, code), INDEX idx_task_priorities_tenant (tenantId))`,
    `CREATE TABLE IF NOT EXISTS task_statuses (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NULL, sourceType ENUM('PLATFORM', 'TENANT') NOT NULL DEFAULT 'PLATFORM', platformMasterId VARCHAR(50) NULL, code VARCHAR(50), name VARCHAR(100), color VARCHAR(50), isTerminal TINYINT(1) NOT NULL DEFAULT 0, isActive TINYINT(1) NOT NULL DEFAULT 1, displayOrder INT NOT NULL DEFAULT 0, masterScopeKey VARCHAR(70) GENERATED ALWAYS AS (COALESCE(tenantId, '__PLATFORM__')) STORED, UNIQUE KEY uq_task_statuses_scope_code (masterScopeKey, code), INDEX idx_task_statuses_tenant (tenantId))`,
    `CREATE TABLE IF NOT EXISTS visit_purposes (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NULL, sourceType ENUM('PLATFORM', 'TENANT') NOT NULL DEFAULT 'PLATFORM', platformMasterId VARCHAR(50) NULL, code VARCHAR(50), name VARCHAR(100), description TEXT, isActive TINYINT(1) NOT NULL DEFAULT 1, displayOrder INT NOT NULL DEFAULT 0, masterScopeKey VARCHAR(70) GENERATED ALWAYS AS (COALESCE(tenantId, '__PLATFORM__')) STORED, UNIQUE KEY uq_visit_purposes_scope_code (masterScopeKey, code), INDEX idx_visit_purposes_tenant (tenantId))`,
    `CREATE TABLE IF NOT EXISTS visit_statuses (id VARCHAR(50) PRIMARY KEY, tenantId VARCHAR(50) NULL, sourceType ENUM('PLATFORM', 'TENANT') NOT NULL DEFAULT 'PLATFORM', platformMasterId VARCHAR(50) NULL, code VARCHAR(50), name VARCHAR(100), color VARCHAR(50), isTerminal TINYINT(1) NOT NULL DEFAULT 0, isActive TINYINT(1) NOT NULL DEFAULT 1, displayOrder INT NOT NULL DEFAULT 0, masterScopeKey VARCHAR(70) GENERATED ALWAYS AS (COALESCE(tenantId, '__PLATFORM__')) STORED, UNIQUE KEY uq_visit_statuses_scope_code (masterScopeKey, code), INDEX idx_visit_statuses_tenant (tenantId))`,
